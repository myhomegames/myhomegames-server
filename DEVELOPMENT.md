...
- **Windows:** Node and npm; **`npm run build:win-tray`** and the full build also need **Go 1.21+** on `PATH` (the tray stub `Start-MyHomeGames-Server.exe` is built with `GOOS=windows GOARCH=amd64`). Builds produce **two** zip files:
  - **`npm run build:win-exe`** — only `pkg` for `win-x64` and **`MyHomeGames-<ver>-win-x64-exe.zip`** (Node server executable + `.env` + `server-info.json`) — optional small package for headless use.
  - **`npm run build:win-tray`** — runs **`pkg`** if needed, then **`MyHomeGames-<ver>-win-x64-tray.zip`** — **self-contained**: packaged **`myhomegames-server-*.exe`**, `Start-MyHomeGames-Server.exe`, `MyHomeGames-Server-Tray.ps1`, `README-WINDOWS.txt`, optional `MyHomeGames-Tray.png`, `.env`, `server-info`. Double-click **`Start-MyHomeGames-Server.exe`** for the tray launcher.
  - **`npm run build:win-unified`** — runs **`pkg`** if needed, then builds **`MyHomeGames-<ver>-win-x64-unified.exe`**: a **single** Windows executable that **embeds** the server + tray PS1 + config (and optional PNG), extracts to `%LOCALAPPDATA%\MyHomeGames\server-runtime\<version>\` on first run, and starts the tray. **No zip**; primary distribution for end users who want one file.
  - Full **`npm run build`** creates both Windows zips plus the **unified** `.exe` after the macOS icon step (so `MyHomeGames-Tray.png` can be included in the tray zip and unified payload when the icon is generated).
