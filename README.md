...
- **`content/games/${gameId}/metadata.json`**: Game metadata files. Each game has its own folder with a metadata.json file containing game properties like `title`, `summary`, `year`, `stars`, etc. (the `id` field is derived from the folder name). Executable scripts are stored in **`content/games/${gameId}/scripts/`** as `.sh` or `.bat` files. Script order is defined by a numeric prefix in the filename: `01-label.sh`, `02-another-1.sh` (the number followed by a hyphen; the optional `-1` is the platform id).
